...
import { AuthContext } from "../auth/auth.context"; // Ajusta la ruta
...
function Navbar() {
  ...
  return (
    <nav className="flex items-center justify-between bg-[#141414] text-white px-6 py-4 shadow-lg fixed w-full z-50">
        ...
        {showUserMenu && (
          <div className="absolute right-0 mt-2 w-48 bg-white text-black shadow-xl rounded-lg p-2 animate-fade-in">
            <ul>
              ...
              {isAuthenticated ? (
                <li onClick={() => logout()} className="flex items-center p-2 hover:bg-gray-200 cursor-pointer transition">
      ...
      <animated.div style={menuAnimation} className="absolute left-0 top-0 h-screen w-64 bg-[#003c3c] text-white p-6 shadow-2xl z-50 transition-transform">
